...
只需要在jar包的同目录下创建`application.properties`添加配置，然后重新启动jar包，就可以应用配置
...
- `@ComponentScan("com.atguigu")` 直接指定扫描的路径，`SpringBootApplication`注解已经包含了这个注解
* **配置默认值**
...
```java
@Import(FastsqlException.class) // 导入第三方的类，并在容器中创建bean。默认为全类名 com.alibaba.druid.FastsqlException
@Configuration // 配置类 代替配置文件.配置类本身也是容器中的组件
// @SpringBootConfiguration 功能相同
public class AppConfig {

    @Bean("hihi")  //替代以前spring配置文件中的bean标签 组件在容器中默认名字为方法名
    //默认为单例的 如果想多实例
    @Scope("prototype")
    public User user() {
        User user = new User();
...
* 否则，就放一个User，名叫lisi
* 注意! 如果类只是存在包，而没有创建为bean则，ConditionalOnClass能够触发，ConditionalOnBean不能
**@ConditionalOnBean（value=组件类型，name=组件名字）：判断容器中是否有这个类型的组件，并且名字是指定的值**

```java
//@ConditionalOnMissingClass(value = "com.alibaba.druid.FastsqlException") 如果放在类级别，那没有这个类则整个类失效
...
* **还是要在类上标@ConfigurationProperties绑定属性，只是不需要加@Component**
...
public class Springboot3Application {
	...
	public static void main(String[] args) {
		...
		System.out.println(person.getCats().get("c1"));
		System.out.println(person.getCats().get("c2"));
...
默认输出格式值：`%clr(%d{${LOG\_DATEFORMAT\_PATTERN:-yyyy-MM-dd'T'HH:mm:ss.SSSXXX}}){faint} %clr(${LOG\_LEVEL\_PATTERN:-%5p}) %clr(${PID:- }){magenta} %clr(---){faint} %clr([%15.15t]){faint} %clr(%-40.40logger{39}){cyan} %clr(:){faint} %m%n${LOG\_EXCEPTION\_CONVERSION\_WORD:-%wEx}`
