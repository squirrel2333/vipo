...
![](MySQL索引、性能、设计规范_001.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220616141351236.png)如上图所示，数据库没有索引的情况下，数据`分布在硬盘不同的位置上面`，读取数据时，摆臂需要前后摆动查询数据，这样操作非常消耗时间。如果`数据顺序摆放`，那么也需要从1到6行按顺序读取，这样就相当于进行了6次IO操作，`依旧非常耗时`。如果我们不借助任何索引结构帮助我们快速定位数据的话，我们查找 Col 2 = 89 这条记录，就要逐行去查找、去比较。从Col 2 = 34 开始，进行比较，发现不是，继续下一行。我们当前的表只有不到10行数据，但如果表很大的话，有`上千万条数据`，就意味着要做`很多很多次硬盘I/0`才能找到。现在要查找 Col 2 = 89 这条记录。CPU必须先去磁盘查找这条记录，找到之后加载到内存，再对数据进行处理。这个过程最耗时间就是磁盘I/O（涉及到磁盘的旋转时间（速度较快），磁头的寻道时间(速度慢、费时)）

假如给数据使用 `二叉树` 这样的数据结构进行存储，如下图所示

![](MySQL索引、性能、设计规范_002.png)
...
![](MySQL索引、性能、设计规范_003.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220616152453203.png)我们只在示意图里展示记录的这几个部分：
...
![](MySQL索引、性能、设计规范_004.png)

![](MySQL索引及调优篇.assets/image-20220616152727234.png)把一些记录放到页里的示意图就是：

![](MySQL索引、性能、设计规范_005.png)
...
![](MySQL索引、性能、设计规范_007.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220616155306705.png)
...
![](MySQL索引、性能、设计规范_008.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220616160216525.png)这个过程表明了在对页中的记录进行增删改查操作的过程中，我们必须通过一些诸如 **记录移动** 的操作来始终保证这个状态一直成立：下一个数据页中用户记录的主键值必须大于上一个页中用户记录的主键值。这个过程称为 **页分裂**。

* **给所有的页建立一个目录项。**

由于数据页的 **编号可能是不连续** 的，所以在向 index\_demo 表中插入许多条记录后，可能是这样的效果：

![](MySQL索引、性能、设计规范_009.png)
...
![](MySQL索引、性能、设计规范_010.png)

以页28 为例，它对应 目录项2 ，这个目录项中包含着该页的页号 28 以及该页中用户记录的最小主 键值 5 。我们只需要把几个目录项在物理存储器上连续存储（比如：数组），就可以实现根据主键 值快速查找某条记录的功能了。比如：查找主键值为 20 的记录，具体查找过程分两步：
...
![](MySQL索引、性能、设计规范_011.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220616162944404.png)从图中可以看出来，我们新分配了一个编号为30的页来专门存储目录项记录。这里再次强调 **目录项记录** 和普通的 **用户记录** 的不同点：
...
![](MySQL索引、性能、设计规范_012.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220616171135082.png)从图中可以看出，我们插入了一条主键值为320的用户记录之后需要两个新的数据页：
...
![](MySQL索引、性能、设计规范_013.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220616173512780.png)如图，我们生成了一个存储更高级目录项的 页33 ，这个页中的两条记录分别代表页30和页32，如果用 户记录的主键值在 [1, 320) 之间，则到页30中查找更详细的目录项记录，如果主键值 不小于320 的 话，就到页32中查找更详细的目录项记录。

我们可以用下边这个图来描述它：

![](MySQL索引、性能、设计规范_014.png)
...
![](MySQL索引、性能、设计规范_015.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220616203852043.png)这个B+树与上边介绍的聚簇索引有几处不同：

![](MySQL索引、性能、设计规范_016.png)
...
![](MySQL索引、性能、设计规范_019.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220617151918786.png)如果二级索引中目录项记录的内容只是 `索引列 + 页号` 的搭配的话，那么为 `c2` 列简历索引后的B+树应该长这样：

![](MySQL索引、性能、设计规范_020.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220617152906690.png)如果我们想新插入一行记录，其中 `c1` 、`c2` 、`c3` 的值分别是: `9`、`1`、`c`, 那么在修改这个为 c2 列建立的二级索引对应的 B+ 树时便碰到了个大问题：由于 `页3` 中存储的目录项记录是由 `c2列 + 页号` 的值构成的，`页3` 中的两条目录项记录对应的 c2 列的值都是1，而我们 `新插入的这条记录` 的 c2 列的值也是 `1`，那我们这条新插入的记录到底应该放在 `页4` 中，还是应该放在 `页5` 中？答案：对不起，懵了
...
![](MySQL索引、性能、设计规范_021.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220617154135258.png)这样我们再插入记录`(9, 1, 'c')` 时，由于 `页3` 中存储的目录项记录是由 `c2列 + 主键 + 页号` 的值构成的，可以先把新纪录的 `c2` 列的值和 `页3` 中各目录项记录的 `c2` 列的值作比较，如果 `c2` 列的值相同的话，可以接着比较主键值，因为B+树同一层中不同目录项记录的 `c2列 + 主键`的值肯定是不一样的，所以最后肯定能定位唯一的一条目录项记录，在本例中最后确定新纪录应该被插入到 `页5` 中。
...
![](MySQL索引、性能、设计规范_022.png)

![](MySQL索引及调优篇.assets/image-20220617160325201.png)![](MySQL索引、性能、设计规范_023.png)

![](MySQL索引、性能、设计规范_024.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220617160413479.png)![](MySQL索引、性能、设计规范_025.png)

![](MySQL索引、性能、设计规范_026.png)
...
② InnoDB的数据文件本身就是索引文件，而MyISAM索引文件和数据文件是分离的 ，索引文件仅保存数 据记录的地址。
...
![](MySQL索引、性能、设计规范_027.png)

![](MySQL索引及调优篇.assets/image-20220617161126022.png)![](MySQL索引、性能、设计规范_028.png)
...
![](MySQL索引、性能、设计规范_030.png)

![](MySQL索引及调优篇.assets/image-20220617161946230.png)**加快查找速度的数据结构，常见的有两类：**

(1) 树，例如平衡二叉搜索树，查询/插入/修改/删除的平均时间复杂度都是 `O(log2N)`;

(2)哈希，例如HashMap，查询/插入/修改/删除的平均时间复杂度都是 `O(1)`; (key, value)

![](MySQL索引、性能、设计规范_031.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220617162153587.png)![](MySQL索引、性能、设计规范_032.png)

![](MySQL索引、性能、设计规范_033.png)

![](MySQL索引及调优篇.assets/image-20220617162548697.png)上图中哈希函数h有可能将两个不同的关键字映射到相同的位置，这叫做 碰撞 ，在数据库中一般采用 链 接法 来解决。在链接法中，将散列到同一槽位的元素放在一个链表中，如下图所示：

![](MySQL索引、性能、设计规范_034.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220617162703006.png)实验：体会数组和hash表的查找方面的效率区别
...
![](MySQL索引、性能、设计规范_035.png)

![](MySQL索引及调优篇.assets/image-20220617163202156.png)**Hash索引适用存储引擎如表所示：**
...
![](MySQL索引、性能、设计规范_036.png)

![](MySQL索引、性能、设计规范_037.png)![](MySQL索引及调优篇.assets/image-20220617163619721.png)
...
![](MySQL索引、性能、设计规范_038.png)

![](MySQL索引、性能、设计规范_039.png)

但是特殊情况，就是有时候二叉树的深度非常大，比如：

![](MySQL索引、性能、设计规范_040.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220617164053134.png)为了提高查询效率，就需要 减少磁盘IO数 。为了减少磁盘IO的次数，就需要尽量 降低树的高度 ，需要把 原来“瘦高”的树结构变的“矮胖”，树的每层的分叉越多越好。

### 6.4 AVL树

![](MySQL索引、性能、设计规范_041.png)

![](MySQL索引及调优篇.assets/image-20220617165045803.png)![](MySQL索引、性能、设计规范_042.png)
...
![](MySQL索引、性能、设计规范_043.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220617165124685.png)你能看到此时树的高度降低了，当数据量 N 大的时候，以及树的分叉树 M 大的时候，M叉树的高度会远小于二叉树的高度 (M > 2)。所以，我们需要把树从“瘦高” 变 “矮胖”。
...
![](MySQL索引、性能、设计规范_044.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220617165937875.png)![](MySQL索引、性能、设计规范_045.png)
...
![](MySQL索引、性能、设计规范_046.png)

**再举例1：**

![](MySQL索引、性能、设计规范_047.png)

### 6.6 B+Tree

![](MySQL索引、性能、设计规范_048.png)

* MySQL官网说明：

![](MySQL索引、性能、设计规范_049.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220617170710329.png)**B+ 树和 B 树的差异在于以下几点：**
...
![](MySQL索引、性能、设计规范_050.png)

![](MySQL索引、性能、设计规范_051.png)

![](MySQL索引及调优篇.assets/image-20220617171011102.png)![](MySQL索引、性能、设计规范_052.png)

![](MySQL索引、性能、设计规范_053.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220617171106671.png)![](MySQL索引、性能、设计规范_054.png)

B 树和 B+ 树都可以作为索引的数据结构，在 MySQL 中采用的是 B+ 树。 但B树和B+树各有自己的应用场景，不能说B+树完全比B树好，反之亦然。

**思考题：为了减少IO，索引树会一次性加载吗？**

![](MySQL索引、性能、设计规范_055.png)

**思考题：B+树的存储能力如何？为何说一般查找行记录，最多只需1~3次磁盘IO**

![](MySQL索引、性能、设计规范_056.png)

**思考题：为什么说B+树比B-树更适合实际应用中操作系统的文件索引和数据库索引？**

![](MySQL索引、性能、设计规范_057.png)

**思考题：Hash 索引与 B+ 树索引的区别**

![](MySQL索引、性能、设计规范_058.png)

**思考题：Hash 索引与 B+ 树索引是在建索引的时候手动指定的吗？**

![](MySQL索引、性能、设计规范_059.png)
...
![](MySQL索引、性能、设计规范_060.png)

### 附录：算法的时间复杂度

同一问题可用不同算法解决，而一个算法的质量优劣将影响到算法乃至程序的效率。算法分析的目的在 于选择合适算法和改进算法。

![](MySQL索引、性能、设计规范_061.png)

# 第7章\_InnoDB数据存储结构

## 1. 数据库的存储结构：页

![](MySQL索引、性能、设计规范_062.png)

### 1.1 磁盘与内存交互基本单位：页

![](MySQL索引、性能、设计规范_063.png)

![](MySQL索引及调优篇.assets/image-20220617193033971.png)![](MySQL索引、性能、设计规范_064.png)

### 1.2 页结构概述

![](MySQL索引、性能、设计规范_065.png)
...
![](MySQL索引、性能、设计规范_066.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220617194256988.png)![](MySQL索引、性能、设计规范_067.png)
...
![](MySQL索引、性能、设计规范_068.png)

如下表所示：

![](MySQL索引、性能、设计规范_069.png)
...
![](MySQL索引、性能、设计规范_070.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220620221112635.png)

当我们从页结构来理解 B+ 树的结构的时候，可以帮我们理解一些通过索引进行检索的原理：

![](MySQL索引、性能、设计规范_071.png)

![](MySQL索引、性能、设计规范_072.png)![](MySQL索引及调优篇.assets/image-20220620221242561.png)
...
![](MySQL索引、性能、设计规范_073.png)

### 4.2 为什么要有段？

![](MySQL索引、性能、设计规范_074.png)

### 4.3 为什么要有碎片区？

![](MySQL索引、性能、设计规范_075.png)
...
处于FREE、FREE\_FRAG 以及 FULL\_FRAG 这三种状态的区都是独立的，直属于表空间。而处于 FSEG 状态的区是附属于某个段的。

如果把表空间比作是一个集团军，段就相当于师，区就相当于团。一般的团都是隶属于某个师的，就像是处于 FSEG 的区全部隶属于某个段，而处于 FREE、FREE\_FRAG 以及 FULL\_FRAG 这三种状态的区却直接隶属于表空间，就像独立团直接听命于军部一样。

## 5. 表空间

![](MySQL索引、性能、设计规范_076.png)
...
```
你能看到 innodb\_file\_per\_table=ON, 这就意味着每张表都会单词保存一个 .ibd 文件。
...
![](MySQL索引、性能、设计规范_078.png)

这些系统表也称为 `数据字典`，它们都是以 B+ 树的形式保存在系统表空间的某个页面中。其中 `SYS\_TABLES、SYS\_COLUMNS、SYS\_INDEXES、SYS\_FIELDS` 这四个表尤其重要，称之为基本系统表 (basic system tables) ，我们先看看这4个表的结构：

![](MySQL索引、性能、设计规范_079.png)

![](MySQL索引、性能、设计规范_080.png)

![](MySQL索引及调优篇.assets/image-20220621151139759.png)![](MySQL索引、性能、设计规范_081.png)

![](MySQL索引、性能、设计规范_082.png)

注意：用户不能直接访问 InnoDB 的这些内部系统表，除非你直接去解析系统表空间对应文件系统上的文件。不过考虑到查看这些表的内容可能有助于大家分析问题，所以在系统数据库 `information\_schema` 中提供了一些以 `innodb\_sys` 开头的表:
...
```
在 `information\_scheme` 数据库中的这些以 `INNODB\_SYS` 开头的表并不是真正的内部系统表 (内部系统表就是我们上边以 `SYS` 开头的那些表)，而是在存储引擎启动时读取这些以 `SYS` 开头的系统表，然后填充到这些以 `INNODB\_SYS` 开头的表中。以 `INNODB\_SYS` 开头的表和以 `SYS` 开头的表中的字段并不完全一样，但仅供大家参考已经足矣。
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220621135638283.png)![](MySQL索引、性能、设计规范_083.png)

**2. 随机读取**

![](MySQL索引、性能、设计规范_084.png)

![](MySQL索引及调优篇.assets/image-20220621135719847.png)![](MySQL索引、性能、设计规范_085.png)

**3. 顺序读取**

![](MySQL索引、性能、设计规范_086.png)
...
![](MySQL索引及调优篇.assets/image-20220621202759576.png)

**2. 唯一性索引**

![](MySQL索引及调优篇.assets/image-20220621202850551.png)

**3. 主键索引**

![](MySQL索引及调优篇.assets/image-20220621203302303.png)

**4. 单列索引**

![](MySQL索引及调优篇.assets/image-20220621203333925.png)

**5. 多列 (组合、联合) 索引**

![](MySQL索引及调优篇.assets/image-20220621203454424.png)

**6. 全文检索**

![](MySQL索引及调优篇.assets/image-20220621203645789.png)

**7. 补充：空间索引**

![](MySQL索引及调优篇.assets/image-20220621203736098.png)
...
可以看到，查询id和name字段时，使用了名称为MultiIdx的索引，如果查询 (name, age) 组合或者单独查询name和age字段，会发现结果中possible\_keys和key值为NULL, 并没有使用在t3表中创建的索引进行查询。
...
由结果可以看到，info字段上已经成功建立了一个名为futxt\_idx\_info的FULLTEXT索引。
...
可以看到，test5表的geo字段上创建了名称为spa\_idx\_geo的空间索引。注意创建时指定空间类型字段值的非空约束，并且表的存储引擎为MyISAM。
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220622224124267.png)

从结果可以看出，索引仍然是默认的升序

在MySQL 8.0版本中查看数据表ts1的结构，结果如下：

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220622224205048.png)
...
![](MySQL索引及调优篇.assets/image-20220623154615702.png)
...
你能看到 SQL 查询效率有了提升，同时显示出来的 student\_id 还是按照递增的顺序 进行展示的。这是因为索引会对数据按照某种顺序进行排序，所以在去重的时候也会快很多。
...
![](MySQL索引及调优篇.assets/image-20220623175306282.png)

#### 8. 使用字符串前缀创建索引

![](MySQL索引及调优篇.assets/image-20220623175513439.png)
...
![](MySQL索引及调优篇.assets/image-20220627151947786.png)
...
因为我们是按照 student\_id 来进行检索的，所以不需要对其他字段创建索引，即使这些字段出现在SELECT字段中。
...
我们知道，通过 idx\_name\_birthday\_phone\_number 索引就可以对 name 列进行快速搜索，再创建一 个专门针对 name 列的索引就算是一个 冗余索引 ，维护这个索引只会增加维护的成本，并不会对搜索有 什么好处。
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220627162248635.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220627162345815.png)

我们可以通过观察了解数据库整体的运行状态，通过性能分析工具可以让我们了解执行慢的SQL都有哪些，查看具体的SQL执行计划，甚至是SQL执行中的每一步的成本代价，这样才能定位问题所在，找到了问题，再采取相应的行动。

**详细解释一下这张图：**

![](MySQL索引及调优篇.assets/image-20220627164046438.png)

![](MySQL索引及调优篇.assets/image-20220627164114562.png)
...
如果我们想要查看某条SQL语句的查询成本，可以在执行完这条SQL语句之后，通过查看当前会话中的`last\_query\_cost`变量值来得到当前查询的成本。它通常也是我们`评价一个查询的执行效率`的一个常用指标。这个查询成本对应的是`SQL 语句所需要读取的读页的数量`。

我们依然使用第8章的 student\_info 表为例：
...
![](MySQL索引及调优篇.assets/image-20220628173022699.png)

### 4.1 开启慢查询日志参数

**1. 开启 slow\_query\_log**
...
![](MySQL索引及调优篇.assets/image-20220628173525966.png)

我们可以看到 `slow\_query\_log=OFF`，我们可以把慢查询日志打开，注意设置变量值的时候需要使用 global，否则会报错：
...
![](MySQL索引及调优篇.assets/image-20220628175226812.png)

你能看到这时慢查询分析已经开启，同时文件保存在 `/var/lib/mysql/atguigu02-slow.log` 文件 中。

**2. 修改 long\_query\_time 阈值**
...
![](MySQL索引及调优篇.assets/image-20220628175353233.png)
...
![](MySQL索引及调优篇.assets/image-20220628175425922.png)

**补充：配置文件中一并设置参数**

如下的方式相较于前面的命令行方式，可以看做是永久设置的方式。

修改 `my.cnf` 文件，[mysqld] 下增加或修改参数 `long\_query\_time、slow\_query\_log` 和 `slow\_query\_log\_file` 后，然后重启 MySQL 服务器。
...
**步骤2：设置参数 log\_bin\_trust\_function\_creators**
...
![](MySQL索引及调优篇.assets/image-20220628195650079.png)
...
![](MySQL索引及调优篇.assets/image-20220628195821440.png)
...
![](MySQL索引及调优篇.assets/image-20220628203545536.png)
...
![](MySQL索引及调优篇.assets/image-20220628204922556.png)
...
![](MySQL索引及调优篇.assets/image-20220628205029208.png)
...
![](MySQL索引及调优篇.assets/image-20220628205243769.png)
...
![](MySQL索引及调优篇.assets/image-20220628205317257.png)

```
mysql> show profile cpu,block io for query 2
```

![](MySQL索引及调优篇.assets/image-20220628205354230.png)
...
不过SHOW PROFILE命令将被启用，我们可以从 information\_schema 中的 profiling 数据表进行查看。

## 6. 分析查询语句：EXPLAIN

### 6.1 概述

![](MySQL索引及调优篇.assets/image-20220628210837301.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220628211207436.png)
...
![](MySQL索引及调优篇.assets/image-20220628211351678.png)
...
![](MySQL索引及调优篇.assets/image-20220628212029574.png)

EXPLAIN 语句输出的各个列的作用如下：

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220628212049096.png)
...
**2. 设置参数 log\_bin\_trust\_function\_creators**
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220628221143339.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220628221414097.png)
...
![](MySQL索引及调优篇.assets/image-20220628221948512.png)

```
mysql > EXPLAIN SELECT * FROM s1 WHERE key1 = 'a';
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220628222055716.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220628222251309.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220629165122837.png)

![](MySQL索引及调优篇.assets/image-20220629170848349.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220629165603072.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220629165909340.png)

![](MySQL索引及调优篇.assets/image-20220629171104375.png)

```
mysql> EXPLAIN SELECT * FROM s1 UNION ALL SELECT * FROM s2;
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220629171138065.png)
...
![](MySQL索引及调优篇.assets/image-20220629171611716.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220629171442624.png)

具体分析如下：

* SIMPLE
查询语句中不包含`UNION`或者子查询的查询都算作是`SIMPLE`类型，比方说下边这个单表查询`select\_type`的值就是`SIMPLE`:

```
mysql> EXPLAIN SELECT * FROM s1;
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220629171840300.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220629171904912.png)

* PRIMARY
对于包含`UNION、UNION ALL`或者子查询的大查询来说，它是由几个小查询组成的，其中最左边的那个查询的`select\_type`的值就是`PRIMARY`,比方说：

```
mysql> EXPLAIN SELECT * FROM s1 UNION SELECT * FROM s2;
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220629171929924.png)
从结果中可以看到，最左边的小查询`SELECT \* FROM s1`对应的是执行计划中的第一条记录，它的`select\_type`的值就是`PRIMARY`。

* UNION
对于包含`UNION`或者`UNION ALL`的大查询来说，它是由几个小查询组成的，其中除了最左边的那个小查询意外，其余的小查询的`select\_type`值就是UNION，可以对比上一个例子的效果。
* UNION RESULT
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220629172449267.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220629172525236.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220629172555603.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220629172622893.png)
...
* MATERIALIZED
当查询优化器在执行包含子查询的语句时，选择将子查询物化之后的外层查询进行连接查询时，该子查询对应的`select\_type`属性就是DERIVED，比如下边这个查询：

```
mysql> EXPLAIN SELECT * FROM s1 WHERE key1 IN (SELECT key1 FROM s2);
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220629172646367.png)
...
![](MySQL索引及调优篇.assets/image-20220629190304966.png)
...
![](MySQL索引及调优篇.assets/image-20220629190335371.png)
...
完整的访问方法如下： `system ， const ， eq\_ref ， ref ， fulltext ， ref\_or\_null ， index\_merge ， unique\_subquery ， index\_subquery ， range ， index ， ALL` 。
...
```
 ![](MySQL索引及调优篇.assets/image-20220630164434315.png)
...
```
 ![](MySQL索引及调优篇.assets/image-20220630164724548.png)
...
```
 ![](MySQL索引及调优篇.assets/image-20220630164802559.png)
从执行计划的结果中可以看出，MySQL打算将s2作为驱动表，s1作为被驱动表，重点关注s1的访问 方法是 `eq\_ref` ，表明在访问s1表的时候可以 `通过主键的等值匹配` 来进行访问。
...
mysql> EXPLAIN SELECT * FROM s1 WHERE key1 = 'a';
```
 ![](MySQL索引及调优篇.assets/image-20220630164930020.png)
...
```
 ![](MySQL索引及调优篇.assets/image-20220630175133920.png)
...
```
 ![](MySQL索引及调优篇.assets/image-20220630175511644.png)
从执行计划的 `type` 列的值是 `index\_merge` 就可以看出，MySQL 打算使用索引合并的方式来执行 对 s1 表的查询。

* `unique\_subquery`
类似于两表连接中被驱动表的`eq\_ref`访问方法，`unique\_subquery`是针对在一些包含`IN`子查询的查询语句中，如果查询优化器决定将`IN`子查询转换为`EXISTS`子查询，而且子查询可以使用到主键进行等值匹配的话，那么该子查询执行计划的`type`列的值就是`unique\_subquery`，比如下边的这个查询语句：

```
mysql> EXPLAIN SELECT * FROM s1 WHERE key2 IN (SELECT id FROM s2 where s1.key1 = s2.key1) OR key3 = 'a';
```
 ![](MySQL索引及调优篇.assets/image-20220630180123913.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220703214407225.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220703214633338.png)
或者：
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220703214657251.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220703214844885.png)
上述查询中的所有列表中只有key\_part2 一个列，而且搜索条件中也只有 key\_part3 一个列，这两个列又恰好包含在idx\_key\_part这个索引中，可是搜索条件key\_part3不能直接使用该索引进行`ref`和`range`方式的访问，只能扫描整个`idx\_key\_part`索引的记录，所以查询计划的`type`列的值就是`index`。
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220703215958374.png)
...
在EXPLAIN语句输出的执行计划中，`possible\_keys`列表示在某个查询语句中，对某个列执行`单表查询时可能用到的索引`有哪些。一般查询涉及到的字段上若存在索引，则该索引将被列出，但不一定被查询使用。`key`列表示`实际用到的索引`有哪些，如果为NULL，则没有使用索引。比方说下面这个查询：

```
mysql> EXPLAIN SELECT * FROM s1 WHERE key1 > 'z' AND key3 = 'a';
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220703220724964.png)

上述执行计划的`possible\_keys`列的值是`idx\_key1, idx\_key3`，表示该查询可能使用到`idx\_key1, idx\_key3`两个索引，然后`key`列的值是`idx\_key3`，表示经过查询优化器计算使用不同索引的成本后，最后决定采用`idx\_key3`。
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704130030692.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704130138204.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704130214482.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704130442095.png)

```
mysql> EXPLAIN SELECT * FROM s1 WHERE key_part1 = 'a' AND key_part2 = 'b';
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704130515031.png)

联合索引中可以比较，key\_len=606的好于key\_len=303
...
![](MySQL索引及调优篇.assets/image-20220704131759630.png)

```
mysql> EXPLAIN SELECT * FROM s1 WHERE key1 = 'a';
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704130837498.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704130925426.png)

```
mysql> EXPLAIN SELECT * FROM s1 INNER JOIN s2 ON s2.key1 = UPPER(s1.key1);
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704130957359.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704131050496.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704131323242.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704131644615.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704132345383.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704132458978.png)

* `Using where` ![](MySQL索引及调优篇.assets/image-20220704140148163.png)

```
mysql> EXPLAIN SELECT * FROM s1 WHERE common_field = 'a';
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704132655342.png) ![](MySQL索引及调优篇.assets/image-20220704140212813.png)

```
mysql> EXPLAIN SELECT * FROM s1 WHERE key1 = 'a' AND common_field = 'a';
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704133130515.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704134324354.png)

* `Using index`
当我们的查询列表以及搜索条件中只包含属于某个索引的列，也就是在可以使用覆盖索引的情况下，在`Extra`列将会提示该额外信息。比方说下边这个查询中只需要用到`idx\_key1`而不需要回表操作:

```
mysql> EXPLAIN SELECT key1 FROM s1 WHERE key1 = 'a';
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704134931220.png)
...
```
 ![](MySQL索引及调优篇.assets/image-20220704140344015.png) ![](MySQL索引及调优篇.assets/image-20220704140411033.png)

```
mysql> EXPLAIN SELECT * FROM s1 WHERE key1 > 'z' AND key1 LIKE '%b';
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704140441702.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704140815955.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704142059555.png)
...
如果出现`Using sort\_union(...)`提示，说明准备使用`Sort-Union`索引合并的方式执行查询。

```
mysql> EXPLAIN SELECT * FROM s1 WHERE key1 = 'a' OR key3 = 'a';
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704142552890.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704142754394.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704142901857.png) ![](MySQL索引及调优篇.assets/image-20220704145143170.png)

```
mysql> EXPLAIN SELECT * FROM s1 ORDER BY common_field LIMIT 10;
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704143518857.png)
需要注意的是，如果查询中需要使用`filesort`的方式进行排序的记录非常多，那么这个过程是很耗费性能的，我们最好想办法`将使用文件排序的执行方式改为索引进行排序`。

* `Using temporary` ![](MySQL索引及调优篇.assets/image-20220704145924130.png)

```
mysql> EXPLAIN SELECT DISTINCT common_field FROM s1;
```

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704150030005.png)
再比如：
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704150156416.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704150308189.png)
从 `Extra` 的 `Using index` 的提示里我们可以看出，上述查询只需要扫描 `idx\_key1` 索引就可以搞 定了，不再需要临时表了。
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704161702384.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704164236909.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704172833362.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704172920158.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704173012413.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704173045190.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704173108888.png)

我们使用 # 后边跟随注释的形式为大家解释了 `EXPLAIN FORMAT=JSON` 语句的输出内容，但是大家可能 有疑问 "`cost\_info`" 里边的成本看着怪怪的，它们是怎么计算出来的？先看 s1 表的 "`cost\_info`" 部 分：
...
小贴士： rows和filter都是我们前边介绍执行计划的输出列，在JSON格式的执行计划中，rows 相当于rows\_examined\_per\_scan，filtered名称不变。
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704174401970.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704174543663.png)
...
![](MySQL索引及调优篇.assets/image-20220704175711800.png)
...
![](MySQL索引及调优篇.assets/image-20220704190726180.png)
...
![](MySQL索引及调优篇.assets/image-20220704192020603.png)
...
![](MySQL索引及调优篇.assets/image-20220704202453482.png)
...
![](MySQL索引及调优篇.assets/image-20220704204140589.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704211116351.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704211254581.png)
...
![](MySQL索引及调优篇.assets/image-20220704212354041.png)

如果此时再插入一条主键值为 9 的记录，那它插入的位置就如下图：

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704212428607.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704214905412.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704215159768.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704215533871.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704215600507.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704215658526.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704215721216.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704220123647.png)

2. 那么索引 idx\_age\_classId\_name 这个索引还能正常使用么？
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704223211981.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704224552374.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704224916117.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220704225333199.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705131643304.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705131717329.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705132221045.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705132239232.png)

因为age字段和name字段上都有索引，所以查询中使用了索引。你能看到这里使用到了`index\_merge`，简单来说index\_merge就是对age和name分别进行了扫描，然后将这两个结果集进行了合并。这样做的好处就是`避免了全表扫描`。
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705145225852.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705160504018.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705160935109.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705161243838.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705161515545.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705161602362.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705161746184.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705161843558.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705161929544.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705162009145.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705163833445.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705165559127.png)

可以看到这种方式效率是非常低的，以上述表A数据100条，表B数据1000条计算，则A\*B=10万次。开销统计如下:

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705165646252.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705172315554.png)

驱动表中的每条记录通过被驱动表的索引进行访问，因为索引查询的成本是比较固定的，故mysql优化器都倾向于使用记录数少的表作为驱动表（外表）。

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705172650749.png)

如果被驱动表加索引，效率是非常高的，但如果索引不是主键索引，所以还得进行一次回表查询。相比，被驱动表的索引是主键索引，效率会更高。

#### 4. Block Nested-Loop Join（块嵌套循环连接）

![](MySQL索引及调优篇.assets/image-20220705173047234.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705174005280.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705174250551.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705205050280.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705210708343.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705210839437.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705215436102.png)

**过程二： order by 时不limit,索引失效**

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705215909350.png)

**过程三：order by 时顺序错误，索引失效**

![](MySQL索引及调优篇.assets/image-20220705220033520.png)

**过程四：order by 时规则不一致，索引失效（顺序错，不索引；方向反，不索引）**

![](MySQL索引及调优篇.assets/image-20220705220404802.png)

结论：ORDER BY 子句，尽量使用 Index 方式排序，避免使用 FileSort 方式排序

**过程五：无过滤，不索引**

![](MySQL索引及调优篇.assets/image-20220705221212879.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705222027812.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705222912521.png)

这样我们优化掉了 using filesort

查询结果如下：

![](MySQL索引及调优篇.assets/image-20220705222954971.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705223111883.png)
...
![](MySQL索引及调优篇.assets/image-20220705223329164.png)
...
![](MySQL索引及调优篇.assets/image-20220705224410340.png)

**2. 尝试提高 max\_length\_for\_sort\_data**

![](MySQL索引及调优篇.assets/image-20220705224505668.png)

\*\*3. Order by 时select  *是一个大忌。最好只Query需要的字段。*

![](MySQL索引及调优篇.assets/image-20220705224551104.png)
...
![](MySQL索引及调优篇.assets/image-20220705225329130.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705225625166.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220705225654124.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706124528680.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706124612180.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706125113658.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706125351116.png)

### 8.2 覆盖索引的利弊

![](MySQL索引及调优篇.assets/image-20220706125943936.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706130901307.png)

以及

![](MySQL索引及调优篇.assets/image-20220706130921934.png)
...
![](MySQL索引及调优篇.assets/image-20220706131320477.png)
...
![](MySQL索引及调优篇.assets/image-20220706135436316.png)

![](MySQL索引及调优篇.assets/image-20220706135506409.png)

* 主键索引 (简图)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706135633814.png)

二级索引zip\_last\_first (简图，这里省略了数据页等信息)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706135701187.png)

![](MySQL索引及调优篇.assets/image-20220706135723203.png)

### 10.4 开启和关闭ICP性能对比

![](MySQL索引及调优篇.assets/image-20220706135904713.png)

![](MySQL索引及调优篇.assets/image-20220706140213382.png)

### 10.5 ICP的使用条件

1. 如果表的访问类型为 range 、 ref 、 eq\_ref 或者 ref\_or\_null 可以使用ICP。
...
![](MySQL索引及调优篇.assets/image-20220706141957185.png)
...
![](MySQL索引及调优篇.assets/image-20220706142648452.png)
...
4. **交互多**
业务还需要额外执行一次类似 last\_insert\_id() 的函数才能知道刚才插入的自增值，这需要多一次的 网络交互。在海量并发的系统中，多1条SQL，就多一次性能上的开销。
5. **局部唯一性**
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706151506580.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706151715106.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706161436920.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706162131362.png)
...
可以通过MySQL8.0提供的uuid\_to\_bin函数实现上述功能，同样的，MySQL也提供了bin\_to\_uuid函数进行转化：
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706162657448.png)
...
![](MySQL索引及调优篇.assets/image-20220706162947613.png)
...
![](MySQL索引、性能、设计规范_087.png)

![](MySQL索引及调优篇.assets/image-20220706164201695.png)![](MySQL索引、性能、设计规范_088.png)
...
![](MySQL索引、性能、设计规范_089.png)

### 2.3 键和相关属性的概念

![](MySQL索引、性能、设计规范_090.png)
...
![](MySQL索引、性能、设计规范_091.png)

该表不符合 1NF ，因为规则说“表的每个属性必须具有原子（单个）值”，lisi和zhaoliu员工的 emp\_mobile 值违反了该规则。为了使表符合 1NF ，我们应该有如下表数据：

![](MySQL索引、性能、设计规范_092.png)

**举例2：**

user 表的设计不符合第一范式

![](MySQL索引、性能、设计规范_093.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706171225292.png)其中，user\_info字段为用户信息，可以进一步拆分成更小粒度的字段，不符合数据库设计对第一范式的 要求。将user\_info拆分后如下：

![](MySQL索引、性能、设计规范_094.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706171242455.png)**举例3：**
...
![](MySQL索引、性能、设计规范_095.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220706171442919.png)表2：

![](MySQL索引、性能、设计规范_096.png)
...
`比赛表 player\_game` ，里面包含球员编号、姓名、年龄、比赛编号、比赛时间和比赛场地等属性，这 里候选键和主键都为（球员编号，比赛编号），我们可以通过候选键（或主键）来决定如下的关系：
...
![](MySQL索引、性能、设计规范_097.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707122639894.png)这样的话，每张数据表都符合第二范式，也就避免了异常情况的发生。
...
![](MySQL索引、性能、设计规范_098.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707123038469.png)违反了第二范式，因为有非主键属性仅依赖于候选键（或主键）的一部分。例如，可以仅通过orderid找 到订单的 orderdate，以及 customerid 和 companyname，而没有必要再去使用productid。

修改：

Orders表和OrderDetails表如下，此时符合第二范式。

![](MySQL索引、性能、设计规范_099.png)
...
![](MySQL索引、性能、设计规范_100.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707124011654.png)商品类别名称依赖于商品类别编号，不符合第三范式。

修改：

表1：符合第三范式的 `商品类别表` 的设计

![](MySQL索引、性能、设计规范_101.png)

表2：符合第三范式的 `商品表` 的设计

![](MySQL索引、性能、设计规范_102.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707124058174.png)商品表goods通过商品类别id字段（category\_id）与商品类别表goods\_category进行关联。

**举例3：**

`球员player表` ：球员编号、姓名、球队名称和球队主教练。现在，我们把属性之间的依赖关系画出来，如下图所示:

![](MySQL索引、性能、设计规范_103.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707124136228.png)

你能看到球员编号决定了球队名称，同时球队名称决定了球队主教练，非主属性球队主教练就会传递依 赖于球员编号，因此不符合 3NF 的要求。

如果要达到 3NF 的要求，需要把数据表拆成下面这样：

![](MySQL索引、性能、设计规范_104.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707124152312.png)**举例4：**
...
![](MySQL索引、性能、设计规范_105.png)

符合3NF后的数据模型通俗地讲，2NF和3NF通常以这句话概括：“每个非键属性依赖于键，依赖于 整个键，并且除了键别无他物”。

### 2.7 小结

![](MySQL索引、性能、设计规范_106.png)

## 3. 反范式化

### 3.1 概述

![](MySQL索引、性能、设计规范_107.png)
...
员工的信息存储在 `employees 表` 中，部门信息存储在 `departments 表` 中。通过 employees 表中的 department\_id字段与 departments 表建立关联关系。如果要查询一个员工所在部门的名称：
...
```
如果经常需要进行这个操作，连接查询就会浪费很多时间。可以在 employees 表中增加一个冗余字段 department\_name，这样就不用每次都进行连接操作了。

**举例2：**

反范式化的 `goods商品信息表` 设计如下：

![](MySQL索引、性能、设计规范_108.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707125118808.png)**举例3：**

我们有 2 个表，分别是 `商品流水表（atguigu.trans ）`和 `商品信息表 （atguigu.goodsinfo）` 。商品流水表里有 400 万条流水记录，商品信息表里有 2000 条商品记录。

商品流水表：

![](MySQL索引、性能、设计规范_109.png)

商品信息表：

![](MySQL索引、性能、设计规范_110.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707125447317.png)新的商品流水表如下所示：

![](MySQL索引、性能、设计规范_111.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707125500378.png)**举例4：**

`课程评论表 class\_comment` ，对应的字段名称及含义如下：

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707125531172.png)

`学生表 student` ，对应的字段名称及含义如下：

![](MySQL索引及调优篇.assets/image-20220707125545891.png)

在实际应用中，我们在显示课程评论的时候，通常会显示这个学生的昵称，而不是学生 ID，因此当我们 想要查询某个课程的前 1000 条评论时，需要关联 class\_comment 和 student这两张表来进行查询。
...
![](MySQL索引及调优篇.assets/image-20220707125642908.png)

运行时长为 0.395 秒，对于网站的响应来说，这已经很慢了，用户体验会非常差。

如果我们想要提升查询的效率，可以允许适当的数据冗余，也就是在商品评论表中增加用户昵称字段， 在 class\_comment 数据表的基础上增加 stu\_name 字段，就得到了 class\_comment2 数据表。
...
![](MySQL索引及调优篇.assets/image-20220707125718469.png)
...
![](MySQL索引及调优篇.assets/image-20220707131428597.png)
...
根据 BCNF 的要求，我们需要把仓库管理关系 warehouse\_keeper 表拆分成下面这样：
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707132038425.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707132344634.png)

导师表：

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707132355841.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707133830721.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707134028730.png)

以及

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707134220820.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707134636225.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707154400580.png)
...
![](MySQL索引及调优篇.assets/image-20220707163807205.png)

进货单表：

![](MySQL索引及调优篇.assets/image-20220707163828614.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707164128704.png)

进货单明细表：

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707164146216.png)

商品信息表：

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707164227845.png)
...
![](MySQL索引及调优篇.assets/image-20220707165011050.png)

进货单头表：

![](MySQL索引及调优篇.assets/image-20220707165038108.png)

这2个表都满足第三范式的要求了。

### 7.4 反范式化：业务优先的原则

![](MySQL索引及调优篇.assets/image-20220707165459547.png)
...
![](MySQL索引及调优篇.assets/image-20220707164146216.png)

商品信息表：

![](MySQL索引及调优篇.assets/image-20220707164227845.png)

这样一来，我们就避免了冗余数据，而且还能够满足业务的需求，这样的数据库设计，才是合格的设计。

## 8. ER模型

![](MySQL索引及调优篇.assets/image-20220707170027637.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707170608782.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707171022246.png)
...
`用户实体`转换成用户表(user\_info)的代码如下所示。

![](MySQL索引及调优篇.assets/image-20220707171335255.png)

![](MySQL索引及调优篇.assets/image-20220707171412363.png)

![](MySQL索引及调优篇.assets/image-20220707171915637.png)

**下面我们再把弱实体转换成数据表：**

![](MySQL索引及调优篇.assets/image-20220707172033399.png)

![](MySQL索引及调优篇.assets/image-20220707172052236.png)

![](MySQL索引及调优篇.assets/image-20220707172143793.png)

![](MySQL索引及调优篇.assets/image-20220707172217772.png)

![](MySQL索引及调优篇.assets/image-20220707172236606.png)

![](MySQL索引及调优篇.assets/image-20220707172259143.png)

#### 2. 一个多对多的关系转换成一个数据表

![](MySQL索引及调优篇.assets/image-20220707172350226.png)

#### 3. 通过外键来表达1对多的关系

![](MySQL索引及调优篇.assets/image-20220707172609833.png)

#### 4. 把属性转换成表的字段

![](MySQL索引及调优篇.assets/image-20220707172819174.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707172918017.png)
...
![](MySQL索引及调优篇.assets/image-20220707173028203.png)

**2. 数据表中的字段个数越少越好**

![](MySQL索引及调优篇.assets/image-20220707173402491.png)

**3. 数据表中联合主键的字段个数越少越好**

![](MySQL索引及调优篇.assets/image-20220707173522971.png)

**4. 使用主键和外键越多越好**

![](MySQL索引及调优篇.assets/image-20220707173557568.png)
...
7. 【建议】临时库以 tmp *为前缀，并以日期为后缀； 备份库以 bak* 为前缀，并以日期为后缀。
...
9. 【强制】布尔值类型的字段命名为 is\_描述 。如member表上表示是否为enabled的会员的字段命 名为 is\_enabled。
...
(1)强制要求主键为id，类型为int或bigint，且为 auto\_increment 建议使用unsigned无符号型。
(2)标识表里每一行主体的字段不要设为主键，建议 设为其他字段如user\_id，order\_id等，并建立unique key索引。因为如果设为主键且主键值为随机 插入，则会导致innodb内部页分裂和大量随机I/O，性能下降。
...
15. 【建议】中间表（或临时表）用于保留中间结果集，名称以 tmp *开头。 备份表用于备份或抓取源表快照，名称以 bak* 开头。中间表和备份表定期清理。 1
16. 【示范】一个较为规范的建表语句：
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707180254510.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707194228064.png)

点开的页面如下所示，name和code已经从概念模型1改成物理模型1了

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707194248236.png)

完成后如下图所示，将自行打开修改的物理模型，需要注意的是这些表的数据类型已经自行改变了，而 且中间表出现两个主键，即双主键

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707194308595.png)

### 11.5 物理模型转为概念模型

上面介绍了概念模型转物理模型，下面介绍一下物理模型转概念模型（如下图点击操作即可）

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707194405358.png)

然后出现如下图所示界面，然后将物理修改为概念 ，点击应用确认即可

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707194419360.png)

点击确认后将自行打开如下图所示的页面，自己观察有何变化，如果转换为oracle的，数据类型会发生变 化，比如Varchar2等等）；

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707194433407.png)

### 11.6 物理模型导出SQL语句

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707194544714.png)

打开之后如图所示，修改好存在sql语句的位置和生成文件的名称即可

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707194557554.png)

在Selection中选择需要导出的表，然后点击应用和确认即可

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707194637242.png)

完成以后出现如下图所示，可以点击Edit或者close按钮

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707194727849.png)
...
![](MySQL索引及调优篇.assets/image-20220707200915836.png)

如何确定呢？一般情况下，有如下几种方式：

![](MySQL索引及调优篇.assets/image-20220707201133424.png)
...
![](MySQL索引及调优篇.assets/image-20220707201443229.png)

#### 第2步：优化表设计

![](MySQL索引及调优篇.assets/image-20220707201617799.png)

#### 第3步：优化逻辑查询

![](MySQL索引及调优篇.assets/image-20220707202059972.png)

#### 第4步：优化物理查询

物理查询优化是在确定了逻辑查询优化之后，采用物理优化技术（比如索引等），通过计算代价模型对 各种可能的访问路径进行估算，从而找到执行方式中代价最小的作为执行计划。**在这个部分中，我们需要掌握的重点是对索引的创建和使用。**

![](MySQL索引及调优篇.assets/image-20220707202156660.png)
...
![](MySQL索引及调优篇.assets/image-20220707202436467.png)

#### 第6步：库级优化

![](MySQL索引及调优篇.assets/image-20220707202555506.png)

![](MySQL索引及调优篇.assets/image-20220707202732911.png)

![](MySQL索引及调优篇.assets/image-20220707203538155.png)
...
![](MySQL索引及调优篇.assets/image-20220707203818987.png)
...
![](MySQL索引及调优篇.assets/image-20220707204403406.png)

* innodb\_buffer\_pool\_size ：这个参数是Mysql数据库最重要的参数之一，表示InnoDB类型的 表 和索引的最大缓存 。它不仅仅缓存 索引数据 ，还会缓存 表的数据 。这个值越大，查询的速度就会越 快。但是这个值太大会影响操作系统的性能。
* key\_buffer\_size ：表示 索引缓冲区的大小 。索引缓冲区是所有的 线程共享 。增加索引缓冲区可 以得到更好处理的索引（对所有读和多重写）。当然，这个值不是越大越好，它的大小取决于内存 的大小。如果这个值太大，就会导致操作系统频繁换页，也会降低系统性能。对于内存在 4GB 左右 的服务器该参数可设置为 256M 或 384M 。
* table\_cache ：表示 同时打开的表的个数 。这个值越大，能够同时打开的表的个数越多。物理内 存越大，设置就越大。默认为2402，调到512-1024最佳。这个值不是越大越好，因为同时打开的表 太多会影响操作系统的性能。
* query\_cache\_size ：表示 查询缓冲区的大小 。可以通过在MySQL控制台观察，如果 Qcache\_lowmem\_prunes的值非常大，则表明经常出现缓冲不够的情况，就要增加Query\_cache\_size 的值；如果Qcache\_hits的值非常大，则表明查询缓冲使用非常频繁，如果该值较小反而会影响效 率，那么可以考虑不用查询缓存；Qcache\_free\_blocks，如果该值非常大，则表明缓冲区中碎片很 多。MySQL8.0之后失效。该参数需要和query\_cache\_type配合使用。
* query\_cache\_type 的值是0时，所有的查询都不使用查询缓存区。但是query\_cache\_type=0并不 会导致MySQL释放query\_cache\_size所配置的缓存区内存。
+ 当query\_cache\_type=1时，所有的查询都将使用查询缓存区，除非在查询语句中指定 SQL\_NO\_CACHE ，如SELECT SQL\_NO\_CACHE \* FROM tbl\_name。
+ 当query\_cache\_type=2时，只有在查询语句中使用 SQL\_CACHE 关键字，查询才会使用查询缓 存区。使用查询缓存区可以提高查询的速度，这种方式只适用于修改操作少且经常执行相同的 查询操作的情况。
* sort\_buffer\_size ：表示每个 需要进行排序的线程分配的缓冲区的大小 。增加这个参数的值可以 提高 ORDER BY 或 GROUP BY 操作的速度。默认数值是2 097 144字节（约2MB）。对于内存在4GB 左右的服务器推荐设置为6-8M，如果有100个连接，那么实际分配的总共排序缓冲区大小为100 × 6 ＝ 600MB。
* join\_buffer\_size = 8M ：表示 联合查询操作所能使用的缓冲区大小 ，和sort\_buffer\_size一样， 该参数对应的分配内存也是每个连接独享。
* read\_buffer\_size ：表示 每个线程连续扫描时为扫描的每个表分配的缓冲区的大小（字节） 。当线 程从表中连续读取记录时需要用到这个缓冲区。SET SESSION read\_buffer\_size=n可以临时设置该参 数的值。默认为64K，可以设置为4M。
* innodb\_flush\_log\_at\_trx\_commit ：表示 何时将缓冲区的数据写入日志文件 ，并且将日志文件 写入磁盘中。该参数对于innoDB引擎非常重要。该参数有3个值，分别为0、1和2。该参数的默认值 为1。
...
* innodb\_log\_buffer\_size ：这是 InnoDB 存储引擎的 事务日志所使用的缓冲区 。为了提高性能， 也是先将信息写入 Innodb Log Buffer 中，当满足 innodb\_flush\_log\_trx\_commit 参数所设置的相应条 件（或者日志缓冲区写满）之后，才会将日志写到文件（或者同步到磁盘）中。
* max\_connections ：表示 允许连接到MySQL数据库的最大数量 ，默认值是 151 。如果状态变量 connection\_errors\_max\_connections 不为零，并且一直增长，则说明不断有连接请求因数据库连接 数已达到允许最大值而失败，这是可以考虑增大max\_connections 的值。在Linux 平台下，性能好的 服务器，支持 500-1000 个连接不是难事，需要根据服务器性能进行评估设定。这个连接数 不是越大 越好 ，因为这些连接会浪费内存的资源。过多的连接可能会导致MySQL服务器僵死。
* back\_log ：用于 控制MySQL监听TCP端口时设置的积压请求栈大小 。如果MySql的连接数达到 max\_connections时，新来的请求将会被存在堆栈中，以等待某一连接释放资源，该堆栈的数量即 back\_log，如果等待连接的数量超过back\_log，将不被授予连接资源，将会报错。5.6.6 版本之前默 认值为 50 ， 之后的版本默认为 50 + （max\_connections / 5）， 对于Linux系统推荐设置为小于512 的整数，但最大不超过900。
如果需要数据库在较短的时间内处理大量连接请求， 可以考虑适当增大back\_log 的值。
* thread\_cache\_size ： 线程池缓存线程数量的大小 ，当客户端断开连接后将当前线程缓存起来， 当在接到新的连接请求时快速响应无需创建新的线程 。这尤其对那些使用短连接的应用程序来说可 以极大的提高创建连接的效率。那么为了提高性能可以增大该参数的值。默认为60，可以设置为 120。
...
当 Threads\_cached 越来越少，但 Threads\_connected 始终不降，且 Threads\_created 持续升高，可 适当增加 thread\_cache\_size 的大小。
...
![](MySQL索引及调优篇.assets/image-20220707210351452.png)

**(1) 调整系统参数 InnoDB\_flush\_log\_at\_trx\_commit**

![](MySQL索引及调优篇.assets/image-20220707210447501.png)

**(2)  调整系统参数 InnoDB\_buffer\_pool\_size**

![](MySQL索引及调优篇.assets/image-20220707210555848.png)

**(3) 调整系统参数 InnoDB\_buffer\_pool\_instances**

![](MySQL索引及调优篇.assets/image-20220707210720394.png)

## 3. 优化数据库结构

![](MySQL索引及调优篇.assets/image-20220707211709553.png)

### 3.1 拆分表：冷热数据分离

![](MySQL索引及调优篇.assets/image-20220707211802756.png)

**举例1：** `会员members表` 存储会员登录认证信息，该表中有很多字段，如id、姓名、密码、地址、电 话、个人描述字段。其中地址、电话、个人描述等字段并不常用，可以将这些不常用的字段分解出另一 个表。将这个表取名叫members\_detail，表中有member\_id、address、telephone、description等字段。 这样就把会员表分成了两个表，分别为 `members表` 和 `members\_detail表` 。
...
如果需要查询会员的基本信息或详细信息，那么可以用会员的id来查询。如果需要将会员的基本信息和 详细信息同时显示，那么可以将members表和members\_detail表进行联合查询，查询语句如下：
...
![](MySQL索引及调优篇.assets/image-20220707212800544.png)
...
现在有一个模块需要经常查询带有学生名称（name）、学生所在班级名称（className）、学生班级班 长（monitor）的学生信息。根据这种情况可以创建一个 temp\_student 表。temp\_student表中存储学生名称（stu\_name）、学生所在班级名称（className）和学生班级班长（monitor）信息。创建表的语句如下：
...
以后，可以直接从temp\_student表中查询学生名称、班级名称和班级班长，而不用每次都进行联合查 询。这样可以提高数据库的查询速度。
...
![](MySQL索引及调优篇.assets/image-20220707213524137.png)
...
![](MySQL索引及调优篇.assets/image-20220707214640374.png)
...
TIMESTAMP存储的时间范围1970-01-01 00:00:01 ~ 2038-01\_19-03:14:07。TIMESTAMP使用4字节，DATETIME使用8个字节，同时TIMESTAMP具有自动赋值以及自动更新的特性。
...
![](MySQL索引及调优篇.assets/image-20220707215305640.png)

**② 禁用唯一性检查**

![](MySQL索引及调优篇.assets/image-20220707215356893.png)
...
插入数据之前执行`set unique\_checks=0`来禁止对唯一索引的检查，数据导入完成之后再运行`set unique\_check=1`。这个和MyISAM引擎的使用方法一样。

**② 禁用外键检查**

![](MySQL索引及调优篇.assets/image-20220707220034534.png)

**③ 禁止自动提交**

![](MySQL索引及调优篇.assets/image-20220707220131891.png)

### 3.6 使用非空约束

![](MySQL索引及调优篇.assets/image-20220707220157606.png)
...
* Msg\_type: 表示信息类型，其值通常是状态 (status) 、信息 (info) 、注意 (note) 、警告 (warning) 和 错误 (error) 之一。
* Msg\_text: 显示信息。
...
其中，tbl\_name是表名；option参数有5个取值，分别是QUICK、FAST、MEDIUM、EXTENDED和 CHANGED。各个选项的意义分别是：
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707221707254.png)

该语句对于检查的表可能会产生多行信息。最后一行有一个状态的 Msg\_type 值，Msg\_text 通常为 OK。 如果得到的不是 OK，通常要对其进行修复；是 OK 说明表已经是最新的了。表已经是最新的，意味着存 储引擎对这张表不必进行检查。
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707221901664.png)
...
![](MySQL索引及调优篇.assets/image-20220707222156765.png)

**方式二：使用mysqlcheck命令**

![](MySQL索引及调优篇.assets/image-20220707222305302.png)
...
![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707222606097.png)

* 双主双从模式：

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707222623485.png)

### 4.3 垂直拆分

当数据量级达到 `千万级` 以上时，有时候我们需要把一个数据库切成多份，放到不同的数据库服务器上， 减少对单一数据库服务器的访问压力。

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707222648112.png)

* 如果数据库的数据表过多，可以采用`垂直分库`的方式，将关联的数据库部署在同一个数据库上。
* 如果数据库中的列过多，可以采用`垂直分表`的方式，将一张数据表分拆成多张数据表，把经常一起使用的列放在同一张表里。

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707222910740.png)
...
![](MySQL索引及调优篇.assets/image-20220707222954304.png)

![](MySQL%E7%B4%A2%E5%BC%95%E5%8F%8A%E8%B0%83%E4%BC%98%E7%AF%87.assets/image-20220707222739120.png)

![](MySQL索引及调优篇.assets/image-20220707223024163.png)
...
![](MySQL索引及调优篇.assets/image-20220707223246684.png)

![](MySQL索引及调优篇.assets/image-20220707223349879.png)

### 5.3 MySQL 8.0新特性：隐藏索引对调优的帮助

![](MySQL索引及调优篇.assets/image-20220707223420496.png)
