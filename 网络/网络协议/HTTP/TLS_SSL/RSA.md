...
传统的 TLS 握手基本都是使用 RSA 算法来实现密钥交换的，在将 TLS 证书部署服务端时，证书文件其实就是服务端的公钥**(发送给客户端的公钥附在证书文件\*.crt中)**，会在 TLS 握手阶段传递给客户端，而服务端的**私钥(.key文件)**则一直留在服务端，一定要确保私钥不能被窃取。
...
1. 服务器的私钥(\*.key)由服务器自身产生
2. 私钥创建公钥(\*.cer)
3. 私钥存储在服务器端的keystore(\*.jks)中

**CA 签发证书**的过程，如上图左边部分：

* 首先 CA 会把持有者的公钥、用途、颁发者、有效时间等信息打成一个包(\*.csr)，然后对这些信息进行 Hash 计算(**签名算法)**，得到一个 Hash 值；
* 然后 CA 会使用自己的私钥(浏览器或计算机上内置的CA私钥)将该 Hash 值加密，生成 Certificate Signature，也就是 CA 对证书做了签名；
* 最后将 Certificate Signature 添加在文件证书上，形成数字证书(.crt)；

客户端校验服务端的数字证书的过程，如上图右边部分：

* 首先客户端会使用同样的 Hash 算法获取该证书的 Hash 值 H1；
* 通常浏览器和操作系统中集成了 CA 的公钥信息，浏览器收到证书(\*.crt)后可以使用 CA 的公钥解密 Certificate Signature 内容，得到一个 Hash 值 H2 ；
