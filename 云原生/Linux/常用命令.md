...
**如果对一个目录有rwx权限，那不管下面的文件有什么权限，都可以对他创建删除。**
...
#查找包含init的文件，使用通配符 \*(匹配任意字符) ?(匹配单个字符)
